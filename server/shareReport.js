  ...
  <main class="shell">
    <header class="topbar">
      <div>
        ...
        <p class="subtle">Snapshot generated <span id="generated-at">${escapeHtml(new Date(generatedAt).toLocaleString())}</span></p>
      </div>
      <div class="summary" id="summary">${summaryHtml(stats)}</div>
    </header>
    ...
    <section class="layout">
      <section class="results">
        <div class="section-head">
          <div>
            <h2>Listings</h2>
            <p id="result-count" class="small">${escapeHtml(`${data.listings.length.toLocaleString()} of ${data.listings.length.toLocaleString()} listings`)}</p>
          </div>
        </div>
        <div id="listing-list" class="listing-list">${listingsHtml(data.listings)}</div>
      </section>
...
function summaryHtml(stats) {
  return [
    ...
    ['Within 30 min', stats.within],
    ['Facing known', stats.facingKnown],
    ['Unread alerts', stats.unread],
    ['Sources', stats.sources],
    ['Road cache', `${stats.roadCacheReady}/${stats.roadCacheTotal}`],
...
function listingsHtml(listings) {
  return listings.length
    ...
    : '<div class="empty">No listings in this snapshot.</div>';
}
...
function notificationCardHtml(item) {
  return `<article class="notification"><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.message)}</p><div class="notification-footer"><span>${escapeHtml(new Date(item.created_at).toLocaleString())}</span>${item.url ? `<a href="${escapeAttr(item.url)}" target="_blank" rel="noreferrer">Open source</a>` : ''}</div></article>`;
}

function sourceLabel(listing) {
  return (listing.source_refs || []).map((ref) => ref.label || ref.source).filter(Boolean).join(', ') || 'Unknown source';
}

function formatMoney(value) {
  return value ? `$${Number(value).toLocaleString()}` : 'Price TBD';
}

function formatFact(value, label) {
  return value ? `${Number(value).toLocaleString()} ${label}` : `${label} unknown`;
}

function yearBuiltLabel(value) {
  return value ? `built ${value}` : 'year built unknown';
}

function distanceLabel(value) {
  return value === null || value === undefined ? 'distance unknown' : `${Number(value).toFixed(1)} mi`;
}

function commuteLabel(listing) {
  return listing.commute_status === 'unknown_commute' ? 'commute unknown' : `${Math.round(listing.commute_minutes || 0)} min`;
}

function facingLabel(listing) {
